...
export function useDatabase() {
  ...
  const insertarLecturas = async (lecturas: Lectura[]): Promise<boolean> => {
    ...
    let sql2 = `INSERT INTO registros(fecha, minutos, activaciones, pitch, roll, refPitch, refRoll, malaPostura)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;

    const prepared = await db.prepareAsync(sql2);
    try {
      await db.withExclusiveTransactionAsync(async () => {
        lecturas.forEach(async (v) => {
    ...
    try {
      ...
      const exec = statement.executeAsync(data);
    } catch (e) {
      console.log(e);
    }
