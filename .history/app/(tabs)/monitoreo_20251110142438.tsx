...
async function logDatabase() {
  ...
  const rows = await db.getAllAsync("SELECT * FROM registros ORDER BY fecha DESC;");
  console.log("📄 Datos en la base:", rows);
}
...
export default function MonitoreoScreen() {
  ...
  useEffect(() => {
  (async () => {
    const db = await SQLite.openDatabase("postura.db");
    const rows = await db.exec("SELECT * FROM registros;");
    ...
    <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.controlButtons}>
        ...
        <Button
          ...
          onPress={() => {
            console.log(
              "Guardado manual (se usará el hook useDatabase internamente)"
            );
          }}
        >
          Registrar ahora
