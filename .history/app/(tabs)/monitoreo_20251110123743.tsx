...
import { useSyncPostura } from "@/hooks/useSyncPostura"; // <── hook de conexión y DB

const { width } = Dimensions.get("window");

export default function MonitoreoScreen() {
  // Dirección IP del ESP32
  const ESP_IP = "192.168.1.42";

  const { connected, posturaCorrecta, lectura, activaciones, minutos } = useSyncPostura(ESP_IP);
  ...
  const textoPostura = posturaCorrecta ? "Postura correcta" : "Postura incorrecta";
  ...
  return (
    <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.estadoPostura}>
        <View style={[styles.circuloExterior, { backgroundColor: colorPostura }]}>
        ...
        <Text variant="caption" style={styles.textoHora}>
          ...
          {ultimaMedicion.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
        </Text>
      </View>
      ...
      {lectura && (
        <View style={styles.datosContainer}>
          ...
          <Text style={styles.dato}>Ref Pitch: {lectura.refPitch.toFixed(2)}°</Text>
          <Text style={styles.dato}>Ref Roll: {lectura.refRoll.toFixed(2)}°</Text>
        </View>
      )}
      ...
      <View style={styles.alertasContainer}>
        ...
        <View style={styles.alertasLista}>
          <View style={styles.alertaFila}>
            ...
            <Text style={styles.alertaHora}>{minutos}</Text>
          </View>
          <View style={styles.alertaFila}>
            <Text style={styles.alertaTipo}>Alertas totales</Text>
            <Text style={styles.alertaHora}>{activaciones}</Text>
          </View>
        </View>
      </View>

      {/* Botón manual para registrar */}
      <Button
        size="lg"
        variant="default"
        style={styles.botonTest}
        onPress={() => {
          console.log("Guardado manual (se usará el hook useDatabase internamente)");
        }}
      >
        Registrar ahora
      </Button>
